...
export default function MovieCard({ posterPath, title, voteAverage, releaseDate, category, match }: MovieCardProps) {
  ...
  return (
    <div className="group/card relative aspect-[2/3] rounded-md overflow-hidden cursor-pointer transition-all duration-300 hover:z-10 hover:scale-105 hover:shadow-xl shadow-black/50 bg-card">
      <img 
        ...
        onError={() => setImgSrc("https://images.unsplash.com/photo-1598899134739-24c46f58b8c0?auto=format&fit=crop&w=800&q=80")}
      />
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
        <div className="translate-y-4 group-hover/card:translate-y-0 transition-transform duration-300">
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            {match && <span className="text-xs font-bold text-green-400">{match} Match</span>}
            <Badge variant="outline" className="text-[10px] h-5 px-1.5 border-gray-500 text-gray-300">{voteAverage.toFixed(1)}</Badge>
            <span className="text-xs text-gray-300">{new Date(releaseDate).getFullYear()}</span>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex gap-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" className="w-8 h-8 rounded-full bg-white text-black hover:bg-gray-200 hover:text-black">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" variant="outline" className="w-8 h-8 rounded-full border-gray-400 bg-transparent text-white hover:bg-white/20 hover:text-white">
                    <Plus className="w-4 h-4" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent><p>Add to My List</p></TooltipContent>
              </Tooltip>
            </div>
            ...
            <Tooltip>
              ...
              <TooltipContent><p>I like this</p></TooltipContent>
            </Tooltip>
          </div>
