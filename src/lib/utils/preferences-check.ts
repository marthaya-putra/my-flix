...
export function hasSufficientPreferences(preferences: UserPreferences): boolean {
  return preferences.movies.length >= 3 && preferences.tvs.length >= 3;
}
...
export function getPreferenceCounts(preferences: UserPreferences) {
  return {
    ...
    total: preferences.movies.length + preferences.tvs.length + preferences.actors.length + preferences.directors.length,
  };
}